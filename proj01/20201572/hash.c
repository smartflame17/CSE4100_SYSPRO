...
/* custom hash function implementation using MurmurHash3 */
unsigned hash_int_2 (int key){
	key ^= key >> 16;
	key *= 0x85ebca6b;	// large prime number constant
	key ^= key >> 13;
	key *= 0xc2b2ae35;	// another prime constant
	key ^= key >> 16;
	return key;
}
...
unsigned my_hash_func(struct hash_elem* e, void* aux){
	...
	return hash_int(item->key);
}

/* Used to compare hash elements. True if a < b, else false */
bool cmp_hash(struct hash_elem* a, struct hash_elem* b, void* aux){
	...
	if(a_item->value < b_item->value) return true;
	else return false;
}
...
// squares hash entry data
void hash_square(struct hash_elem* e, void* aux){
	struct hash_item* item = hash_entry(e, struct hash_item, elem);
	int square = item->value;
	item->value = square * square;
}

// triples hash entry data
void hash_triple(struct hash_elem* e, void* aux){
	struct hash_item* item = hash_entry(e, struct hash_item, elem);
	int triple = item->value;
	item->value = triple * triple * triple;
}

// frees hash_item that e is pointing to
